...
def run_assistant_graph(
    ...
    for idx, node in enumerate(agent_nodes):
        ...
        for tool_info in agent_tool_configs:
            if tool_info.get("kind") == "user_tool":
                ...
                if template_key:
                    ...
                    # It can be stored as {"tavily": {"api_key": "..."}} or {"api_key": "..."}
                    ...
                    if isinstance(raw_config, dict):
                        ...
                        elif "api_key" in raw_config or any(k.startswith("_") for k in raw_config.keys()):
                            # Flat: {"api_key": "..."} - use as is
        ...
        print(f"[DEBUG] Agent {agent_id} tool_configs: {tool_configs}")
